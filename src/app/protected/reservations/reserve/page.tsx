...
export default function ReservePage() {
  ...
  const {session, addRental } = useSession();
  ...
  const loadRoom = useCallback(async () => {
    const room = session?.data.rooms.find(r => r.id === roomId);
    setRoom(room);
  }, [session, roomId]);
  ...
  const handleReserve = () => {
    const uuid = Math.floor(Math.random() * 1_000_000).toString().padStart(6, '0').toString();
    ...
    addRental(rental);
    router.push(`/protected/reservations/confirmation?uuid=${uuid}`);
